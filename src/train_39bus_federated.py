...
class GAT_NodeLocalizer(torch.nn.Module):
    def __init__(self, in_channels, out_channels=1):
        ...
        # 1 GAT head for lightweight CPU execution
...
def inspect_dataset_labels(df):
    ...
    total_nodes = len(df)
    total_snapshots = total_nodes // 39
    
    snapshot_labels = df['Snapshot_Label'].values[::39]
    ...
    print(f"  - Secure Nodes (Class 0): {num_normal_nodes} ({num_normal_nodes/total_nodes*100:.4f}%)", flush=True)
    print(f"  - Attacked Nodes (Class 1): {num_attack_nodes} ({num_attack_nodes/total_nodes*100:.4f}%)", flush=True)
    print(f"  - Node Imbalance Ratio (Secure/Attack): {num_normal_nodes/max(num_attack_nodes, 1):.2f}", flush=True)
...
def train_federated():
    ...
    print(f"[Federated GNN 39-Bus] Using device: {device}", flush=True)
    
    print("Loading IEEE 39-bus data...", flush=True)
    df = pd.read_csv('data/ieee39_extracted.csv')
    edges_df = pd.read_csv('data/grid_topology_39bus_dynamic.csv')
    ...
    total_snapshots = len(df) // 39
    indices = np.arange(total_snapshots)
    snapshot_labels = df['Snapshot_Label'].values[::39]
    
    # Stratified Split (80% train, 20% test)
    train_idx, test_idx = train_test_split(indices, test_size=0.2, random_state=39, stratify=snapshot_labels)
    train_labels = snapshot_labels[train_idx]
    train_idx_c1, train_idx_c2 = train_test_split(train_idx, test_size=0.5, random_state=39, stratify=train_labels)
    ...
    y_graph_tensor = torch.tensor(df['Snapshot_Label'].values[::39], dtype=torch.float).view(-1, 1)
    
    print("Pre-building dynamic edge index tensors for all snapshots...", flush=True)
    edge_index_by_step = {}
    grouped = edges_df.groupby('Time_Step')
    ...
    df_topo_static = pd.read_csv("data/grid_topology_39bus.csv")
    ...
    def get_data_tensors(snapshot_indices):
        ...
        for i in snapshot_indices:
            node_indices.extend(list(range(i*39, (i+1)*39)))
    ...
    # =========================================================================
    print("\n" + "="*80, flush=True)
    print("TASK 1: TRAINING RESGAT NODE LOCALIZER (39-BUS)", flush=True)
    print("="*80, flush=True)
    ...
    torch.save(global_model_node.state_dict(), "models/trained_gat_39bus.pth")
    
    # =========================================================================
    # Task 2: Train Graph Classifier Model
    # =========================================================================
    print("\n" + "="*80, flush=True)
    print("TASK 2: TRAINING RESGAT GRAPH DETECTION CLASSIFIER (39-BUS)", flush=True)
    print("="*80, flush=True)
    ...
    torch.save(global_model_graph.state_dict(), "models/trained_gat_39bus_graph.pth")
    ...
    plt.tight_layout()
    plt.savefig("plots/federated_gat_39bus_results.png", dpi=150, bbox_inches='tight')
    plt.close()
