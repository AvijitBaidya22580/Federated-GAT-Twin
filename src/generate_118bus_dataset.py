...
def generate_ieee118_dataset():
    ...
    for t in range(num_snapshots):
        ...
        # Decide if this snapshot has a line outage (15% probability)
        has_outage = np.random.rand() < 0.15
        outage_line = None
        if has_outage:
            # Disconnect a random line to simulate an outage event
        ...
        try:
            ...
            # If power flow fails to converge, restore and run with all lines in service
        ...
        if not success:
            # Skip this snapshot if it didn't converge at all
            continue
        ...
        i_line = np.nan_to_num(i_line, nan=0.0)
        ...
        # Create a list of line connections for each bus
        bus_currents = {b: [] for b in range(num_buses)}
        for idx in range(len(from_buses)):
            if outage_line is not None and idx == outage_line:
                cur = 0.0
            else:
                cur = i_line[idx]
            from_b = from_buses[idx]
        ...
        # Record active edges for this time step
        for idx in range(len(from_buses)):
            if outage_line is None or idx != outage_line:
                from_b = from_buses[idx]
                to_b = to_buses[idx]
                dynamic_edges_rows.append([t, from_b, to_b])
                dynamic_edges_rows.append([t, to_b, from_b])
        ...
        for b in range(num_buses):
            ...
            snort_log = 0.0
            if outage_line is not None:
                outage_from = from_buses[outage_line]
                outage_to = to_buses[outage_line]
    ...
    edges_df.to_csv('data/grid_topology_118bus_dynamic.csv', index=False)
    print(f"Dynamic edge mapping saved to 'data/grid_topology_118bus_dynamic.csv' with {len(edges_df)} entries.")
